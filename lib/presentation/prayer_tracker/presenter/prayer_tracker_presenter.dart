...
import 'package:prayer_times/presentation/common/remove_dialog.dart';
import 'package:prayer_times/presentation/home/models/waqt.dart';
import 'package:prayer_times/presentation/prayer_tracker/presenter/prayer_tracker_ui_state.dart';
...
class PrayerTrackerPresenter extends BasePresenter<PrayerTrackerUiState> {
  ...
  void togglePrayerStatus({required WaqtType type}) {
    ...
    final List<PrayerTrackerModel> trackers = List<PrayerTrackerModel>.from(
  ...
  void handleHorizontalDrag(DragUpdateDetails details) {
    ...
    final double limitedOffset = newOffset.clamp(
      -screenWidthFactor,
      screenWidthFactor,
    );
  ...
  List<PrayerTrackerModel> _getInitialTrackers(DateTime selectedDate) {
    ...
    for (final type in WaqtType.values) {
      final bool isSelectable = _timeService
  ...
  bool canSelectNextDate() {
    ...
    final DateTime nextDate = currentUiState.selectedDate.add(
      const Duration(days: 1),
    );
              ...
              for (final entry in trackerEntries) {
                catchFutureOrVoid(() async {
                  ...
                  if (entry.trackerData != null &&
                    ...
                    final List<PrayerTrackerModel>? trackers = catchAndReturn(
                      () {
